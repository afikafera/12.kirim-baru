import re
from datetime import datetime, timezone

import httpx

from .base import BaseConnector, Document


class BinanceFuturesConnector(BaseConnector):
    """Official Binance USDⓈ-M Futures market-data connector."""

    PATTERN = re.compile(
        r"^https?://(?:www\.)?binance\.com/(?:[a-z]{2}/)?futures/"
        r"([A-Z0-9]+)$",
        re.IGNORECASE,
    )

    API_URL = "https://fapi.binance.com/fapi/v1/ticker/24hr"
    PREMIUM_INDEX_API_URL = "https://fapi.binance.com/fapi/v1/premiumIndex"

    def detect(self, url: str) -> bool:
        return bool(self.PATTERN.search(url))

    def _extract_symbol(self, url: str) -> str | None:
        match = self.PATTERN.search(url)
        if not match:
            return None
        return match.group(1).upper()

    def fetch(self, url: str) -> Document:
        cached = self._cache_get(url)
        if cached:
            return cached

        symbol = self._extract_symbol(url)
        if not symbol:
            return Document(
                title="Error",
                source_url=url,
                platform="binance-futures",
                metadata={"error": "symbol_not_found"},
            )

        try:
            resp = httpx.get(
                self.API_URL,
                params={"symbol": symbol},
                timeout=15,
            )
            resp.raise_for_status()
            data = resp.json()

            if not isinstance(data, dict) or data.get("symbol") != symbol:
                return Document(
                    title="Error",
                    source_url=url,
                    platform="binance-futures",
                    metadata={
                        "error": "symbol_not_found",
                        "symbol": symbol,
                    },
                )

            close_time = data.get("closeTime", "")
            close_utc = ""

            if close_time:
                close_utc = datetime.fromtimestamp(
                    int(close_time) / 1000,
                    tz=timezone.utc,
                ).isoformat()

            # Funding rate & Mark price from premiumIndex (graceful / non-blocking)
            last_funding_rate = ""
            last_funding_rate_percent = ""
            next_funding_time = ""
            next_funding_time_utc = ""
            mark_price = ""
            index_price = ""

            try:
                p_resp = httpx.get(
                    self.PREMIUM_INDEX_API_URL,
                    params={"symbol": symbol},
                    timeout=15,
                )
                if p_resp.status_code == 200:
                    p_data = p_resp.json()
                    if isinstance(p_data, dict):
                        raw_fr = p_data.get("lastFundingRate", "")
                        if raw_fr is not None and str(raw_fr).strip() != "":
                            last_funding_rate = str(raw_fr)
                            try:
                                last_funding_rate_percent = f"{float(raw_fr) * 100:.4f}%"
                            except Exception:
                                last_funding_rate_percent = ""

                        nft = p_data.get("nextFundingTime", "")
                        if nft:
                            next_funding_time = nft
                            try:
                                next_funding_time_utc = datetime.fromtimestamp(
                                    int(nft) / 1000,
                                    tz=timezone.utc,
                                ).isoformat()
                            except Exception:
                                next_funding_time_utc = ""

                        if p_data.get("markPrice") is not None:
                            mark_price = str(p_data.get("markPrice", ""))
                        if p_data.get("indexPrice") is not None:
                            index_price = str(p_data.get("indexPrice", ""))
            except Exception:
                # Failure in premiumIndex must not fail 24hr ticker data
                pass

            doc = Document(
                title=f"Binance Futures: {symbol}",
                source_url=url,
                platform="binance-futures",
                metadata={
                    "symbol": symbol,
                    "last_price": data.get("lastPrice", ""),
                    "price_change": data.get("priceChange", ""),
                    "price_change_percent": data.get(
                        "priceChangePercent", ""
                    ),
                    "high_price": data.get("highPrice", ""),
                    "low_price": data.get("lowPrice", ""),
                    "volume": data.get("volume", ""),
                    "quote_volume": data.get("quoteVolume", ""),
                    "close_time": close_time,
                    "close_time_utc": close_utc,
                    "last_funding_rate": last_funding_rate,
                    "last_funding_rate_percent": last_funding_rate_percent,
                    "next_funding_time": next_funding_time,
                    "next_funding_time_utc": next_funding_time_utc,
                    "mark_price": mark_price,
                    "index_price": index_price,
                    "api_url": self.API_URL,
                },
            )

            self._cache_set(url, doc)
            return doc

        except httpx.HTTPStatusError as e:
            try:
                error_data = e.response.json()
            except Exception:
                error_data = {}

            error_code = error_data.get("code")
            error_message = error_data.get("msg", str(e))

            if error_code == -1121:
                return Document(
                    title="Error",
                    source_url=url,
                    platform="binance-futures",
                    metadata={
                        "error": "invalid_symbol",
                        "symbol": symbol,
                        "binance_code": error_code,
                        "binance_message": error_message,
                    },
                )

            return Document(
                title="Error",
                source_url=url,
                platform="binance-futures",
                metadata={
                    "error": error_message,
                    "symbol": symbol,
                    "binance_code": error_code,
                },
            )

        except Exception as e:
            return Document(
                title="Error",
                source_url=url,
                platform="binance-futures",
                metadata={
                    "error": str(e),
                    "symbol": symbol,
                },
            )
